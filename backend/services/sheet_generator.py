...
def generate_sheet_music_pdf(midi_data: dict, output_path: str):
    ...
    title      = midi_data.get("title", "Transcribed Piano Sheet Music")

    lily_src = _build_lilypond(
        treble_notes, bass_notes, tempo, time_sig, key_sig, use_flats, title
    )
...
def _build_lilypond(treble_notes: list, bass_notes: list,
                    ...
                    key_sig: int, use_flats: bool,
                    title: str = "Transcribed Piano Sheet Music") -> str:
...
\\header {{
  ...
  tagline = "MediaKit · {tempo_int} BPM · {time_sig[0]}/{time_sig[1]}"
}}
...
def _notes_to_voice(note_list: list, tempo: float, time_sig: list,
    ...
    # Phrase-level dynamics: one marking per 4-bar phrase when velocity changes
    phrase_dyns = _compute_phrase_dynamics(events, time_sig)

    measure_dur_beats = float(time_sig[0])
    last_break_measure = 0
    ...
    for idx, ev in enumerate(events):
        ...
        # Phrase-level dynamic (only when phrase changes and velocity shifts)
        if idx in phrase_dyns:
            parts[-1] += f"\\{phrase_dyns[idx]}"
...
def _compute_phrase_dynamics(events: list, time_sig: list) -> dict:
    """
    Return {event_index: dynamic_string} emitting one dynamic per 4-bar phrase,
    only when the average velocity changes enough to warrant a new marking.
    """
    if not events:
        return {}

    phrase_beats = 4.0 * float(time_sig[0])
    phrase_groups: dict = {}   # phrase_idx → [(event_idx, velocity), ...]

    for i, ev in enumerate(events):
        phrase_idx = int(ev["t_beats"] / phrase_beats)
        phrase_groups.setdefault(phrase_idx, []).append((i, ev.get("velocity", 80)))

    result = {}
    prev_dyn = None

    for phrase_idx in sorted(phrase_groups.keys()):
        pairs = phrase_groups[phrase_idx]
        avg_vel = sum(v for _, v in pairs) / len(pairs)
        dyn = _velocity_to_dynamic(int(avg_vel))
        if dyn != prev_dyn:
            result[pairs[0][0]] = dyn   # attach to first event of phrase
            prev_dyn = dyn
